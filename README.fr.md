...
- **Cela tient sur un vrai dépôt de production.** Sur trycompai/comp (un monorepo bun d'environ 1,7k étoiles en développement actif dont le `CLAUDE.md` avait divergé de l'`AGENTS.md` et enseignait encore des commandes `npx` périmées), une seule passe de traitement a fait passer l'agent de **18/24 à 24/24 bonnes réponses** — avant traitement, il récitait le document périmé mot pour mot, de façon déterministe, dans les deux runs — tout en réduisant la latence de 37 % et les tours d'agent de 45 % ; la limite est documentée honnêtement par un **résultat nul** publié sur openai/codex, dont les documents sains ne laissaient rien à améliorer — [cas positif](benchmark/corpus/evals/comp/README.md) · [cas nul](benchmark/corpus/evals/codex/README.md).
...
- **Des résultats dignes de confiance.** 38 tours de validation consignés contre des dépôts réels ont produit quinze corrections de faux positifs (openai/codex, microsoft/vscode, cline, gemini-cli, assistant-ui, …), chacune publiée avec des tests de régression ; les classes différées sont consignées ouvertement, jamais cachées — [journal de validation externe](EXTERNAL_VALIDATION.md).
...
| Couverture | Agents reconnus : Claude Code, Cursor, Windsurf, GitHub Copilot, Gemini CLI, Cline, Roo, Continue, Trae, Amazon Q Developer, JetBrains Junie, Warp, Firebase Studio, Goose, Kiro, Augment Code et Kilo Code. |
| Sécurité | Secrets en clair (AWS, GitHub, OpenAI, Anthropic, Google, HuggingFace, GitLab, npm, PyPI, Slack, Stripe et JWT), permissions trop larges, transports MCP non sûrs, hooks dangereux et contournements. |
| Cohérence | Scripts absents, binaires directs de dépendances, chemins déplacés, drift du gestionnaire, drift de version de runtime (Node.js, Python, Go, Rust, Java, Ruby, .NET), liens cassés, lockfiles concurrents et identifiants non-chemins — comme des règles lint ou refs de branche libellées — sans faux positifs. |
...
L’apply/remove du guard est transactionnel pour le hook Git, les fichiers du provider et `AGENTS.md`.

Les échecs interceptés provoquent un rollback, les mutations interrompues sont récupérées avant le prochain `--apply`, et un état de récupération dangereux ou modifié extérieurement échoue en mode fail-closed.
