...
- **신뢰할 수 있는 결과.** 실제 저장소 대상 38회의 검증 라운드 기록에서 오탐 수정 15건(openai/codex, microsoft/vscode, cline, gemini-cli, assistant-ui 등)이 회귀 테스트와 함께 릴리스되었고, 보류된 유형도 숨기지 않고 공개 기록합니다 — [외부 검증 로그](EXTERNAL_VALIDATION.md).
...
| Coverage | 인식하는 에이전트: Claude Code, Cursor, Windsurf, GitHub Copilot, Gemini CLI, Cline, Roo, Continue, Trae, Amazon Q Developer, JetBrains Junie, Warp, Firebase Studio, Goose, Kiro, Augment Code, Kilo Code. |
| Security | Plaintext secrets (AWS, GitHub, OpenAI, Anthropic, Google, HuggingFace, GitLab, npm, PyPI, Slack, Stripe 자격 증명과 JWT), 과도한 권한, 안전하지 않은 MCP transport, 위험한 hook, bypass 지침. |
| Consistency | 없는 script, 직접 dependency binary, 이동한 path, package manager drift, runtime version drift (Node.js, Python, Go, Rust, Java, Ruby, .NET), broken links, competing lockfiles, 그리고 labeled lint rule 또는 branch ref처럼 오탐을 안전하게 피하는 non-path identifier. |
...
- 인접 단어가 backtick `org/name`을 Docker/OCI image 또는 RPC/API method로 명시적으로 라벨링하면 checked path가 아닌 runtime identifier로 처리합니다. 이 제외는 fail-closed이며, 확장자가 있거나 세 개 이상 세그먼트인 token은 path로 유지합니다.
...
- Secret finding은 값 대신 유형/경로만 보고합니다. 저장소가 제어하는 hook 및 MCP command/URL inventory는 JSON, Markdown, 전체 report 직렬화 전에 redacted 처리되며, 위험한 hook snippet은 SARIF와 PR feedback에서도 redacted 상태를 유지합니다.
