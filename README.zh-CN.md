...
| 一致性 | 缺失脚本、直接依赖二进制命令、移动路径、包管理器漂移、运行时版本漂移、坏链接、多套 lockfile，带标签的 lint rule、branch ref 或 GitHub Actions `uses` 引用等安全排除误报的非路径标识符，以及斜杠枚举的受支持工具列表（`npm/yarn/pnpm`、`jest/vitest/mocha`、`prettier/biome/dprint`）——列出选项而非声明单一使用者，不再制造虚假冲突。 |
