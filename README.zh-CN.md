...
3. **Weekly checkup + deduped issue** — 防止 pull request 之间的慢性腐烂：环境变化、依赖行为变化，或在没有仓库改动时逐渐过期的约定。
...
|---|---|---|
| `github` | `.github/workflows/harness-drift.yml` every-PR gate + `.github/workflows/harness-checkup.yml` weekly scan/drift checkup with a deduped issue。 | 在 GitHub Actions 上自动运行。 |
