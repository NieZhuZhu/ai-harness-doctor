...
| 步骤 | CI 安全？ | 会写入？ | 说明 |
|---|---:|---:|---|
| `scan` | ✅ | 仅使用 `--write-baseline` 时 | 默认以 0 退出；执行清单、证据、安全、缺口、语义一致性、冲突及项目快照检查。`--fail-on-security`/`--fail-on-gaps`/`--fail-on-semantic`/`--fail-on-conflicts` 分别以 2/3/4/7 退出。`--write-baseline` 会显式记录非安全债务。 |
