...
`explain` 会在不修改文件的前提下回答一个聚焦的 scope 问题：哪些 canonical `AGENTS.md` 构成目标的继承链、哪些已识别 config 在诊断上与之关联，以及哪些 scoped override/conflict 与之相关。
...
- 确定性的 scan/explain/plan/validate/stubs/drift/review/eval 脚本需要 Python >=3.9，且只使用 stdlib。
...
| 步骤 | CI 安全？ | 会写入？ | 说明 |
|---|---:|---:|---|
| `scan` | ✅ | 仅使用 `--write-baseline` 时 | 合法的单仓扫描默认以 0 退出；执行清单、证据、安全、缺口、语义一致性、冲突及项目快照检查。`--fail-on-security`/`--fail-on-gaps`/`--fail-on-semantic`/`--fail-on-conflicts` 分别以 2/3/4/7 退出；批量覆盖失败以 8 退出。`--write-baseline` 会显式记录非安全债务。 |
| `explain` | ✅ | ❌ | 为一个已存在或未来的 contained path 解释 canonical inheritance 与 diagnostic scope evidence。 |
| `plan` | ✅ | 可选输出文件 | 搭建合并计划；不会执行合并。 |
| Write `AGENTS.md` | ❌ | ✅ | 由人或 agent 完成的语义步骤。 |
| `validate` | ✅ | ❌ | 检查 canonical path/size/sections，并拒绝未处理的 generated draft markers。 |
...
| `/harness-explain` | 仓库路径 + target path | 展示 canonical chain、diagnostic sources、scoped overrides/conflicts 和 limitations。 | 呈现只读 scope evidence 后。 | scoped guidance 是否符合预期；命令不会修改文件。 |
...
每种 provider 的 merge-request guard 现在都会先运行完整 scan 门禁（`--fail-on-security --fail-on-gaps --fail-on-semantic --fail-on-conflicts`），再检查 drift。如果仓库已经显式审查并提交 `.ai-harness-doctor/scan-baseline.json`，guard 会用它登记既有非安全债务；guard 绝不会创建或更新该文件，HIGH 安全发现始终阻塞。定时 checkup 会同时保留 scan 与 drift 报告，任一门禁失败都会让任务失败。

在 pull request 上，GitHub guard 模板还会多做两件事。其一，在不重复运行门禁的前提下捕获 active `scan --json` 和 `drift --json` 结果，再把两份报告传给同一次 `ai-harness-doctor review`，产出**完整、可操作的 PR 反馈**。安全项、大小警告、gaps、语义不一致、自定义 findings、conflicts 和 drift 都会纳入；monorepo findings 会保留带 package 前缀的仓库相对路径。已 baseline 的 scan 债务仍在 scan JSON 中可见，但不会被重新发布成 active failure。独立的 `--repos-file` 批量 finding 会保留非绝对的仓库标签并只进入 summary，因为它们不能安全地附着到某一个仓库的 PR diff。可定位的单仓 finding 会变成内联评论，包含规则、严重度、发现内容、对 AI agent 的影响、已有证据和修复建议。最终总结会展示健康分/等级、严重度分布、内联与汇总数量、完整 findings 索引、每个 finding 的可折叠完整详情，以及按优先级排列的下一步，并带有稳定标记 `<!-- ai-harness-doctor:pr-review -->`。重复运行时，`--post` 会更新由当前认证用户或 GitHub App 拥有的最新标记总结，而不是不断堆叠完整评论；绝不会编辑他人伪造的标记评论。可重复传入 `--report PATH`，在一次 dry-run 或发布中组合多份报告。内联 findings 仍是与当前 head 绑定的注解。若 GitHub 因内联位置无效而返回 HTTP 422，已经 upsert 的完整总结会作为 fallback，不会被再次发布；权限、网络、限流和服务端错误仍会明确暴露。若报告干净，则说明已覆盖的检查范围并明确无需操作。它默认 dry-run（打印 JSON 负载，绝不触网），仅在 `--post` 时用 `GITHUB_TOKEN` 发布。其二，运行 **eval 证据 + 健康分卡点**。从声明了 `evidence` 的 tasks 生成或重评分的结果，会自动把这些事实来源纳入确定性的证据元数据；可重复的 `--evidence FILE` 用于追加 `AGENTS.md` 等手工来源。`eval --score ... --tasks ... --workdir ... --evidence AGENTS.md --require-current-evidence --fail-under <N>` 会重新推导 task 声明的来源，并先在证据缺失或过期时以 `7` 退出，再应用原有健康阈值（退出码 `5`）。文件证据使用 SHA-256；目录证据只绑定存在性与类型，不递归哈希子项。指纹只证明字节或目录身份一致，并不证明手工答案来自真实模型。通用 guard 仍把 eval 作为可选项；本仓库自举 guard 强制校验已提交结果。所有发布的 guard 命令都通过已打包的 CLI 运行，因此在没有复制 `scripts/` 目录的新消费仓库中也能工作。内联 PR review 反馈仅限 GitHub；GitLab/Codebase 模板共享 scan 与可选 eval 卡点，但不发布 provider-specific 内联评论。

纵深防御，从强到弱：

1. **Pre-commit hard block** — 防止本地改动在离开机器前就让 `AGENTS.md` 过期。`AI_HARNESS_DOCTOR_SKIP=1` 是显式、可审计的绕过，而不是静默放行。
2. **Every-PR gate** — 不使用 `paths` / `paths-ignore` 过滤，防止 hook 被绕过。安全与语义输入包括 MCP/settings 文件、嵌套 agent rules、各生态 manifest，以及 `AGENTS.md` 引用的任意仓库相对路径；任何有限 allow-list 都无法安全覆盖动态的 D2/D7 surface。
3. **Weekly checkup + 生命周期管理的 issue** — 防止 pull request 之间的慢性腐烂：环境变化、依赖行为变化，或在没有仓库改动时逐渐过期的约定。重复失败会把证据追加到一个精确标题的事件 issue；首次恢复健康时会追加恢复证据并关闭它，其他 issue 不受影响。
...
想在一个已经 drift 的仓库上启用 gate？先用 `drift --write-baseline FILE` 记录当前发现，再用 `drift --baseline FILE` 把债务分类为 new、known（`baselined`）和已修复（`resolved_baseline`），这样 CI 只 gate 新增 drift。基线指纹不依赖行号，known/已修复债务都不影响健康分，因此完全基线化的仓库依然是 grade A。用 `--check-baseline` 在已修复条目需要清理时以 `9` 退出，再用 `--prune-baseline` 只原子删除这些条目而不吸收新债务。

### Pre-commit 框架

已经统一使用 [pre-commit](https://pre-commit.com)？可以直接把 drift 与 scan 守护加进你的 `.pre-commit-config.yaml`，而不必运行 `guard --apply`：
...
`ai-harness-doctor-drift` 会在 `AGENTS.md` 与仓库事实漂移时阻塞提交；`ai-harness-doctor-scan` 在出现 HIGH 安全发现时失败。两者都对整个仓库运行（`pass_filenames: false`、`always_run: true`），机器上需具备 Node 与 Python 3。用 `args:` 追加参数（例如用 `["--strict"]` 把 drift notice 升级为错误），单次提交可用 pre-commit 自带的 `SKIP=ai-harness-doctor-drift git commit ...` 跳过。
...
| MCP clients | `ai-harness-doctor mcp` 通过 stdio 将 `harness_scan`/`drift`/`validate`/`plan`/`stubs`/`eval_generate`/`explain` 暴露为 MCP 工具。 |
...
Adapters 会把 `{{PLAYBOOK}}` 替换为已安装 playbook 路径。复制 payload 位于独立的 `.ai-harness-doctor/payload/` 子树，与仓库自有的 baseline、rules 和其他状态分离。版本化的 `~/.ai-harness-doctor/manifest.json` 会记录精确托管路径及 SHA-256 digest。现有 manifest 若损坏、schema 不受支持或状态路径是 symlink，installer 会 fail closed 并保持原样；合法状态通过原子替换写入。install/update/uninstall 通过归属明确的进程锁串行执行，并使用受 containment 保护的持久事务 journal：可捕获的失败会把文件系统与 manifest 一起回滚；若进程中断，下一次严格 installer 命令会恢复尚未提交 manifest 的事务，或通过精确 digest 识别已经提交的 manifest。journal 损坏、恢复状态有歧义，或崩溃后路径被外部修改时会 fail closed 并保留 journal，绝不覆盖恢复证据。这是逻辑 journal 恢复，并不声称跨文件系统写入共享一次原子 rename。install/update 绝不覆盖无归属的同名文件或安装后被用户修改的托管文件，而是报告 `manual-merge` / `modified-preserved`。uninstall 只删除逐字节验证的托管文件，并在最后一个引用它的 agent 被移除前保留共享 payload。旧 manifest 会增量迁移，只清理与旧 payload 逐字节一致的文件。`--link` 会指向全局 package，而不是复制 payload 文件；CLI 会阻止不安全的 `npx` cache linking，并提示你先全局安装。
...
只移除指定 `--agent` 对应且仍与 manifest 所有权记录一致的 Claude skill 文件、slash commands、adapter prompts 和 shared payloads。用户修改或无归属的文件会被保留并报告。`--agent all` 会移除所有已验证的托管 surface。它也会删除匹配的 manifest records。
...
把 manifest 追踪到的每个复制安装重新部署到当前 package 版本，同时保留用户修改过的文件。链接安装会刷新 command pointers，而 payload 会跟随 `npm update -g ai-harness-doctor`。
...
|---|---|---|
| `github` | `.github/workflows/harness-drift.yml` every-PR gate + `.github/workflows/harness-checkup.yml` weekly scan/drift checkup，并维护一个有完整生命周期的 issue。 | 在 GitHub Actions 上自动运行；失败时创建/更新精确事件，恢复时关闭。 |
...
**Self-bootstrap：** 本仓库运行自己的 guard。`.github/workflows/harness-drift.yml` 与 `.github/workflows/harness-checkup.yml` 由 `assets/guard/` 模板改写而来，运行本仓库的**本地** scan 与 drift 实现而非已发布包，因此对 `scripts/` 的改动会被正在改动的代码本身所守护。经审查的 `.ai-harness-doctor/scan-baseline.json` 登记了 benchmark/test fixtures 带来的已知冲突；PR 门禁会因任何新增 scan 债务或 drift 而失败。通用发布版 guard 仍把 eval 作为可选项，但本仓库会无条件以当前 task/`AGENTS.md` 证据和健康分校验已提交的自举 eval。只有 PR review 发布步骤容忍缺失/受限 token。
...
**既有 scan 债务的接入基线。** 仓库可以先记录当前缺口、语义和冲突债务，提交这份透明登记表，之后只对新增发现设门禁：
...
该版本化基线具备确定性、不含时间戳，使用包含发现 family/rule、根或包作用域、路径/证据及 message 的结构化身份，并把行号只视为证据而不是身份。当前债务会被分类为 new（活跃数组）、known（`baselined`）和已修复（`resolved_baseline`）；Markdown 摘要也会展示 known/resolved 数量。所有 baseline debt 都不会参与 fail-on 决策和 SARIF。`--check-baseline` 在已修复条目需要删除时以 `9` 退出（活跃 finding 的 2/3/4/7 仍优先），`--prune-baseline` 则只原子删除已修复条目，绝不会吸收新债务。HIGH 安全发现永远不会进入基线且始终保持活跃；手工伪造的 security 形状条目也会被忽略。普通 scan 遇到缺失或损坏的 baseline 时不会压制任何内容，而显式 check/prune 会 fail closed 且不写文件。Monorepo 的根与各包身份彼此区分。`--repos-file` 刻意不兼容所有 baseline 模式，因为每个无关仓库都应拥有自己的债务登记表。像审代码一样审查基线；它不是正则 ignore 列表。
...
**嵌套指令作用域。** 冲突诊断遵循 AGENTS.md 的最近文件规则，且不会从自然语言中猜测作用域：每个 `AGENTS.md` / `AGENT.md` 的父目录构成一个词法作用域，每个配置文件归属其最深的 canonical 祖先。只有同一作用域内的不同值才会成为阻塞性的 `conflicts`；祖先 → 后代差异会成为非阻塞的 `scope_overrides`，兄弟作用域彼此独立。JSON 新增 `instruction_scopes` 与 `scope_overrides`；Markdown 和 Treat 计划都会展示二者，使有意覆盖可审计且不会被折叠成根 stub。非根作用域的真实冲突会携带 `scope` 进入 baseline、SARIF 和 PR review。本模型不推断文件类型、glob/frontmatter 或自然语言作用域。
...
**多仓库批量模式。** `scan --repos-file PATH` 会扫描 `PATH` 中列出的每个仓库（每行一个路径；空行和 `#` 注释会被忽略），而不是单个 `repo_root`，并打印一份组织级别的健康摘要——面向"多工具混用团队"和"OSS 维护者"这两类人设，他们此前除了对每个仓库手动跑一遍之外没有别的方案。每个仓库都在自己的根目录独立扫描（该模式不会在单个仓库内部展开 monorepo 的包）；一个解析不到目录的路径会被列在"无法扫描的仓库"下，同时不隐藏其他可达仓库的报告。覆盖范围会 fail closed：所有可达仓库扫描完成后，只要有一个条目未被扫描，最终命令就退出 `8`，除非 finding 门禁优先返回 `2`（安全）、`3`（gaps）、`4`（semantic）或 `7`（conflicts）。`--json` 返回 `{ summary: { repo_count, error_count, aggregate }, repos: [{ path, resolved, name, has_agents_md, summary, report } | { path, resolved, error }] }`。批量运行的 operational errors 在 PR review 中只进入 summary，且不会泄露本地 `resolved` 路径。与 `repo_root` 位置参数互斥，也不能与 `--baseline` / `--write-baseline` 组合。

**GitHub 原生发现（SARIF）。** `scan` 和 `drift` 都支持 `--sarif`，将 SARIF 2.1.0 文档输出到 stdout，使发现出现在 GitHub 的 Security 页签以及 PR 内联注释中。`--sarif` 优先于 `--json`/markdown，并基于活跃报告（根 + 每个 monorepo 包）生成，不受任何 `--no-*` 输出抑制影响。Scan SARIF 包含大小/截断警告、安全发现、gaps、语义不一致、conflicts，以及显式 opt-in 的自定义规则发现；drift SARIF 包含内置和自定义 drift findings。文件 inventory、overlap candidates、嵌套文件 inventory 和项目快照仍作为 JSON/Markdown 证据，而不会伪装成 code-scanning findings。合法基线化的非安全债务会被排除；HIGH 安全发现始终保留。源级别映射到 SARIF 级别（`HIGH`/`ERROR`→`error`，`MEDIUM`/`WARN`/`NOTICE`→`warning`，其余→`note`）。每个 result 都携带确定性的、行不敏感的 `partialFingerprints` 条目（由 scan 基线所用的同一套 canonical 发现身份派生），因此无关的编辑或单行位移都会保留同一告警，而不会关闭旧告警并新开一个重复告警。每个命令的 run 还会声明各自的 `automationDetails` category（`ai-harness-doctor/scan/` 与 `ai-harness-doctor/drift/`），因此为同一 commit 同时上传 `scan` 与 `drift` 的 SARIF 时，第二次上传不再关闭第一条命令的告警。确定性的 `properties.aiHarnessDoctor` 属性包让自动化可以直接读取 command、error/warning/note/finding 数量，以及 drift health（`ok`、score、grade）。
...
Action 默认运行所选 Action ref 内自带的实现，因此 `uses:` 指向的版本就是实际执行的代码。只有在明确想覆盖为其他 npm 版本或标签时，才需要设置可选的 `version` 输入。Action 会写 Job Summary，并暴露 `status`（`ok`/`findings`/`maintenance`）、finding/error/warning/note 数量、`resolved-baseline-count` 及 drift `health-score` / `health-grade` outputs；显式选择旧 npm 版本时仍会从标准 SARIF 推导数量，但不可用的 health outputs 保持为空。Doctor 只执行一次：有效的非零 finding 或 baseline-maintenance gate 会先发布 SARIF、outputs 和 summary，再恢复原始 CLI exit code；安装、运行时或 malformed-report 失败不会伪造健康结果。本仓库的 required self-test 会运行当前 bundled `scan` 与 `drift`，再用从公共 registry 解析出的精确稳定版本验证 npm 安装覆盖路径；已发布包调用只证明兼容性，bundled 调用才是 PR 源码的证据。稳定版发布会在 publish 前重复两个 bundled 命令，发布后再通过浮动 Action 运行 bundled scan 和刚发布的精确 npm drift override。每条成功路径都会校验 SARIF driver version，并把 npm 安装限制在 `RUNNER_TEMP`。
...
| `--fail-on-conflicts` | 存在任意 harness 声明冲突时以 `7` 退出。 |
| `--baseline FILE` | 只压制 `FILE` 中登记的 gap/semantic/conflict 债务；安全发现永不可压制。 |
| `--write-baseline FILE` | 把当前非安全 scan 债务写入确定性基线并以 `0` 退出。 |
| `--check-baseline` | 与 `--baseline FILE` 一起使用；存在可删除的已修复/陈旧条目时以 `9` 退出（finding gate 优先）。 |
| `--prune-baseline` | 与 `--baseline FILE` 一起使用；仅原子删除已修复条目，绝不基线化新发现。 |
...
| `--repos-file PATH` | 扫描 `PATH` 中列出的每个仓库；报告全部可达仓库后，只要有条目未被扫描就退出 `8`（除非 2/3/4/7 优先）。与 `repo_root` 互斥。 |
...
每条推断行都标记 `(inferred — confirm)`，安全默认约定标记 `(suggested default)`，顶部横幅提醒人工在提交前审阅并修改。这些产品自有的 provisional markers 也是可执行安全边界：维护者必须处理生成的 TODO、确认或重写每条带标记的 inference/default，并移除横幅/标记；在此之前，`validate` 会报告 `DRAFT_REVIEW` 错误，`stubs --apply` 会拒绝替换原始 instruction sources。它只证明显式 marker 已处理，并不证明 reviewer 身份或语义正确性。不带 `-o` 时 draft 打印到 stdout；带 `-o PATH` 时写入文件，且在文件已存在时拒绝覆盖，除非提供 `--force`。
...
写好 `AGENTS.md` 后验证 canonical readiness：路径安全、大小、必需结构，以及是否仍有 ai-harness-doctor 生成的 provisional markers。它是对 `scripts/canonicalize.py --validate` 的只读透传。
...
默认 dry-run。`--apply` 在任何写入或删除前都会要求 canonical readiness；完整的 pre-migration tool files 仍是非阻塞的 migration inputs。Apply 还要求 git tree 干净。`--force` 只覆盖 dirty-tree 检查，绝不会绕过路径、结构、大小或 `DRAFT_REVIEW` 错误。

已知的工具 config 文件在 `assets/agent-tools.json` 中统一定义，这是 `scan`、`stubs`/`canonicalize` 和 `drift` 共同读取的唯一 registry，因此新增一个工具只需修改这一个文件。

本着同样的精神，`adapters/` 下按命令划分的 Codex/Cursor/Gemini 适配器由单一来源生成：`scripts/gen_adapters.py` 从一张命令表渲染出全部 18 个文件（6 个命令 × 3 种风格），`python3 scripts/gen_adapters.py --check`（亦即 `npm run lint:adapters`）会在任一已提交的适配器与该来源发生漂移时让 CI 失败，`npm run gen:adapters` 则用于重新生成它们。
...
Checks 可以是对提取答案执行的 `regex`、在 workdir 中执行的 `command`，也可以是用于开放式 LLM-as-judge 评分的 `judge`。task 只有在 runner 以 `0` 退出且所选 check 通过时才能通过；非零、timeout 或 spawn failure 会保留为有界的诊断失败 record，绝不会送入 judge。显式 external judge 也必须以 `0` 退出并输出合法且通过的 verdict JSON；非零或 malformed 输出不能通过，也不能静默 fallback 到另一种 judge。single、multi-round 和 matrix 共用这一契约，并会在失败后继续后续 task/agent；若整体 CLI 必须失败，请使用 `--fail-under` 或 regression gate。在 run、multi-round、matrix、regrade 或 strict score 执行其他动作前，完整 task 数组会先通过校验：每条 task 都必须有唯一且非空的 `id`、非空 `prompt`、受支持的 `check` 对象，以及（若存在）有限正数 `timeout_s` 和合法的 evidence/judge 字段。非法 task pack 会以 `2` 退出并给出简短的 `task error`，且发生在任何 runner、LLM/judge、证据哈希、结果写入或 baseline 更新之前；诊断不会回显 prompt 或 task 内容。离线结果消费者（`--score`、`--stats`、`--compare` 和 `--regrade`）还会在任何读取后的副作用或写入前，分别校验已存储的单轮、多轮或矩阵 task records；非法输入会以 `2` 退出并给出简短的 `result error`。对于 Claude CLI JSON 输出，评分会先提取 `result` 字段再匹配。Usage/cost 字段存在时会被捕获。`--compare before.json after.json` 会写出 Markdown 对比。`--regrade results.json --tasks tasks.json` 会离线重新评分已记录输出。提供 `--workdir` 后，run、matrix 和 regrade 会自动盖章 task 级 `evidence` 中列出的全部来源；可重复的 `--evidence FILE` 用于追加手工或额外来源。文件保留精确 SHA-256 指纹；目录只绑定仓库相对路径、存在性和类型，不绑定递归子项字节。评分时，`--require-current-evidence` 会从同一个 `--tasks` 文件重新推导 task 来源，与重复传入的显式证据合并，并在健康检查前拒绝过期结果（退出码 `7`）。既没有声明证据也没有显式证据的手写 tasks 保持旧的无盖章行为；未使用严格标志时，旧的无指纹结果仍可评分。如果 runner binary 缺失，命令会打印手动协议 fallback，而不是假装已经运行。
...
**健康分（Health score）。** 每次 eval 还会计算一个一键式疗效健康分 = 所有任务记录的通过率，以 `0–100` 表示并带 A–F 字母等级（A ≥90 / B ≥80 / C ≥70 / D ≥60 / F）。它以 `health` key 嵌入单次运行结果（`{"tasks":...}`）和矩阵结果（`{"agents":...}`），并打印为一行摘要（`health score: N/100 (grade X), P/T tasks passed`）。超时计为失败。`--score PATH` 会打印某个已有 results/matrix JSON 的健康分（加 `--json` 输出机器可读格式），`--fail-under N` 会在健康分低于 `N` 时以退出码 `5` 退出（作为 CI 门禁）。已存储的 `health` 只是一份一致性缓存：离线消费者总会从已校验的 records 重新推导结论，并拒绝任何与 records 不一致的已存 `score`、等级、计数或比率，因此伪造的缓存分数无法通过阈值或进入 baseline 历史。

**多轮稳定性（`--rounds`）。** `--rounds N`（N > 1）会将整个任务集运行 N 次并汇总稳定性统计，用于暴露那些有时通过、有时失败的 *flaky*（不稳定）任务。此时结果 JSON 会带上 `rounds`、`round_results`（每一轮的完整任务记录及该轮的 `health`）、一个 per-task 的 `task_stats` 数组（`runs`、`passed`、`failed`、`timed_out`、`pass_rate`、`flaky`），以及一个 `stats` 摘要（`mean_health`、`variance`、`stddev`、`min_health`、`max_health`、`health_scores`、`flaky_tasks`、`flaky_count`）。当某任务既非每轮都通过、也非每轮都失败时即为 `flaky`。总体 `health` 为所有 task-run 的通过率，`--fail-under N` 以其为门禁。`--rounds 1`（默认）会逐字节保持旧的单轮输出结构不变。`--stats PATH` 会先校验每一轮和每条 task record，重新推导逐轮与整体健康分并拒绝矛盾的缓存 health，再离线汇总已有的多轮结果文件（加 `--json` 输出机器可读格式，`--fail-under N` 作为门禁）。
...
解释一个 contained file、directory 或 future path 相关的 instruction evidence：
...
- `tools/list` → 公布 `harness_scan`、`harness_drift`、`harness_validate`、`harness_plan`、`harness_stubs`、`harness_eval_generate`、`harness_explain`，各自带 closed input schema。
...
工具参数：`harness_scan`（`json`）、`harness_drift`（`json`、`strict`）、`harness_validate`（`json`）、`harness_plan`、`harness_stubs`、`harness_eval_generate`（可选 `target`）、`harness_explain`（必填 `target`，可选 `json`）。七个工具全部只读；`harness_stubs` 永远不会收到 `--apply`，`harness_eval_generate` 永远不会收到 `-o` 或运行 agent/LLM，`harness_explain` 也不会执行 plugin 或写文件。每个公开的输入 schema 都会在 Python 启动前拒绝缺失的必填字段、未知属性和错误类型。现代协议下还会公布只读、非破坏、幂等、closed-world annotations 以及带类型的结果 envelope `outputSchema`；旧协议会省略其 schema 未定义的字段。元数据 `status` 为 `ok`、`findings` 或 `error`：显式请求且合法的 JSON 发现报告会保留内容并使用 `isError: false`，而非法目标、运行时失败、超时、畸形报告，以及无法安全判定的非零文本报告会设置 `isError: true`。为向后兼容，未 initialize 就直接调用工具的客户端会得到历史 2024 结果格式；合法 initialize handshake 会为整条连接选定 wire version。未知的方法/工具与非法参数会返回 JSON-RPC error 对象。服务器仍只支持 stdio，不会宣称 roots、resources、prompts、sampling 或 HTTP transport。
...
Python 按优先级顺序发现：`AI_HARNESS_DOCTOR_PYTHON`、然后 `PYTHON`、然后 `python3`、然后 `python`；仅接受 Python **3** 解释器。当它缺失时，每个 Python 子命令（`scan`、`explain`、`plan`、`validate`、`stubs`、`drift`、`review`、`eval`）都会以同一条清晰、可操作的信息失败——安装 Python 3 或设置 `AI_HARNESS_DOCTOR_PYTHON`——而不是抛出原始堆栈。
...
- 每次发布都会在 npm publish 前自测该 tag 对应的 Action、移动匹配的浮动 major Action tag（`1.x` 对应 `v1`）、以消费者方式验证它，并创建 Marketplace 确认提醒。
