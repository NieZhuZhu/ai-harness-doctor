...
想在一个已经 drift 的仓库上启用 gate？先用 `drift --write-baseline FILE` 记录当前的所有发现，再用 `drift --baseline FILE` 精确压制这些既有发现，这样 CI 只会因新增 drift 而失败——和 ruff、mypy、detekt 提供的渐进接入方式一致。基线指纹不依赖行号，被压制的发现仍会在 `baselined` 数组中可见，健康分只统计新增 drift，因此完全基线化的仓库依然是 grade A。
...
该版本化基线具备确定性、不含时间戳，使用包含发现 family/rule、根或包作用域、路径/证据及 message 的结构化身份，并把行号只视为证据而不是身份。被压制的债务仍会出现在顶层 `baselined` 数组和 Markdown 基线摘要中，但不会参与 fail-on 决策和 SARIF。HIGH 安全发现永远不会进入基线且始终保持活跃；手工伪造的 security 形状条目也会被忽略。缺失或损坏的基线不会压制任何内容。Monorepo 的根与各包身份彼此区分。`--repos-file` 刻意不兼容 scan baseline，因为每个无关仓库都应拥有自己的债务登记表。像审代码一样审查基线变更，并随着债务修复而缩减文件；它不是正则 ignore 列表。
