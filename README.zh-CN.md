...
在 pull request 上，GitHub guard 模板还会多做两件事。其一，在不重复运行门禁的前提下捕获 active `scan --json` 和 `drift --json` 结果，再把两份报告传给同一次 `ai-harness-doctor review`，产出**完整、可操作的 PR 反馈**。安全项、大小警告、gaps、语义不一致、自定义 findings、conflicts 和 drift 都会纳入；monorepo findings 会保留带 package 前缀的仓库相对路径。已 baseline 的 scan 债务仍在 scan JSON 中可见，但不会被重新发布成 active failure。独立的 `--repos-file` 批量 finding 会保留非绝对的仓库标签并只进入 summary，因为它们不能安全地附着到某一个仓库的 PR diff。可定位的单仓 finding 会变成内联评论，包含规则、严重度、发现内容、对 AI agent 的影响、已有证据和修复建议。最终总结会展示健康分/等级、严重度分布、内联与汇总数量、完整 findings 索引、每个 finding 的可折叠完整详情，以及按优先级排列的下一步，并带有稳定标记 `<!-- ai-harness-doctor:pr-review -->`。可重复传入 `--report PATH`，在一次 dry-run 或发布中组合多份报告。若 GitHub 因内联位置无效而返回 HTTP 422，工具会把这份完整总结改发为普通 PR 评论，确保所有修复指引得以保留；权限、网络、限流和服务端错误仍会明确暴露。若报告干净，则说明已覆盖的检查范围并明确无需操作。它默认 dry-run（打印 JSON 负载，绝不触网），仅在 `--post` 时用 `GITHUB_TOKEN` 发布。其二，通过 `ai-harness-doctor eval --score <已提交的 results.json> --fail-under <N>` 运行 **eval 健康分卡点**，当 eval 健康分低于阈值时使 CI 失败（退出码 5）。所有发布的 guard 命令都通过已打包的 CLI 运行，因此在没有复制 `scripts/` 目录的新消费仓库中也能工作。内联 PR review 反馈仅限 GitHub；GitLab/Codebase 模板同样具备 scan 与 eval 卡点，但不发布 provider-specific 内联评论。
