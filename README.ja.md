...
|---|---:|---:|---|
| `scan` | ✅ | ❌ | デフォルトで 0 で終了します。inventory、evidence、security checkup、不足しているインフラの gap analysis を行います。`--fail-on-security` は HIGH の findings があれば 2 で終了し、`--fail-on-gaps` は ERROR の gap があれば 3 で終了します。 |
...
さらに **gap analysis** を実行し、リポジトリを harness 完全性チェックリストと diff して、*不足している*インフラ（既存のものだけでなく）を報告します。canonical なルート `AGENTS.md`、必須の `AGENTS.md` セクション（`assets/AGENTS.template.md` と同期）、`AGENTS.md` を指す最小 pointer であるべき tool stub、drift-guard / 週次 checkup の CI workflow、pre-commit drift guard、maintenance contract、および MCP / permission 設定です。各 gap は `level`（`ERROR`/`WARN`/`NOTICE`）、`item`、`message`、実行可能な `suggestion` を持ちます。`--fail-on-gaps` を付けると、ERROR レベルの gap（例: ルート `AGENTS.md` の欠落）が 1 つでもあれば `3` で終了します。
...
`security` の findings は `level`（`HIGH`/`MEDIUM`）、`category`（`secret`/`mcp`/`permission`/`hook`/`instruction`）、`path`、および人間が読める `message` を持ちます。`--no-security` では `security` key は省略されます。`gaps` の entries は `check`（`G1`–`G8`）、`level`（`ERROR`/`WARN`/`NOTICE`）、`item`、`message`、`suggestion` を持ちます。`--no-gaps` では `gaps` key は省略されます。
