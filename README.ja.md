...
- **報告される検出結果は信頼できます。** 実リポジトリに対する 38 回の検証ラウンドの記録から、15 件の誤検出修正（openai/codex、microsoft/vscode、cline、gemini-cli、assistant-ui など）が回帰テスト付きでリリースされ、保留中のクラスも隠さず公開記録されています — [外部検証ログ](EXTERNAL_VALIDATION.md)。
...
| Security | Plaintext secrets（AWS、GitHub、OpenAI、Anthropic、Google、OpenRouter、Groq、xAI、Perplexity、HuggingFace、GitLab、npm、PyPI、Slack、Stripe、Tavily、DigitalOcean、Doppler、SendGrid 資格情報と JWT）、広すぎる権限、安全でない MCP transport、危険な hook、権限 bypass 指示。 |
...
`tests/fixtures`、`benchmark/`、`examples/` のような慣例ディレクトリ配下にコミットされた sandbox リポジトリは、ホストリポジトリの live harness surface ではなくテストデータとして扱われます。
...
- Eval task の並行実行は `--parallel N`（`1`～`32`、default `1`）で明示的かつ上限付きで有効化し、結果順序は deterministic に保たれます。
- Eval result artifact と report は runner/judge diagnostics、nested usage metadata、matrix runner template の高信頼 credential を redact します。Numeric usage は保持され、grading は bounded raw output を使い、explicit runner/judge failure と矛盾する stored pass は拒否されます。
