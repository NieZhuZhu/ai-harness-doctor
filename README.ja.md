...
`explain` はファイルを変更せずに、焦点を絞った scope の問いへ答えます。target に適用される canonical `AGENTS.md` chain、診断上関連付けられた認識済み config、関連する scoped override/conflict を表示します。
...
- 決定的な scan/explain/plan/validate/stubs/drift/review/eval scripts には Python >=3.9 が必要です。stdlib-only です。
...
|---|---:|---:|---|
| `scan` | ✅ | `--write-baseline` 使用時のみ | valid な single-repo scan はデフォルトで 0 で終了し、inventory、evidence、security、gap、semantic consistency、conflict、project snapshot を検査します。`--fail-on-security`/`--fail-on-gaps`/`--fail-on-semantic`/`--fail-on-conflicts` は 2/3/4/7、batch coverage failure は 8 で終了します。`--write-baseline` は非 security debt を明示的に記録します。 |
...
| `/harness-explain` | Repo path + target path | canonical chain、diagnostic sources、scoped overrides/conflicts、limitations を表示します。 | read-only scope evidence の提示後。 | scoped guidance が意図通りか。command は編集しません。 |
...
各 provider の merge-request guard は、drift より先に完全な scan gate（`--fail-on-security --fail-on-gaps --fail-on-semantic --fail-on-conflicts`）を実行します。repository が `.ai-harness-doctor/scan-baseline.json` を明示的に review して commit 済みなら、guard は既存の非 security debt にそれを使用します。guard 自身がこの file を作成・更新することはなく、HIGH security finding は常に block します。scheduled checkup は scan と drift の両 report を保持し、どちらかの gate が失敗すれば job も失敗します。

pull request では、GitHub guard テンプレートはさらに 2 つのことを行います。1 つ目は、gate を再実行せず active な `scan --json` と `drift --json` の結果を保存し、両 report を 1 回の `ai-harness-doctor review` に渡して**完全で実行可能な PR feedback**を作ることです。security、size warning、gap、semantic mismatch、custom finding、conflict、drift を含み、monorepo finding は package prefix 付き repository-relative path を保持します。baseline 済み scan debt は scan JSON には残りますが、active failure として再投稿されません。独立した `--repos-file` batch finding は非 absolute な repository label を保持して summary-only になります。これは 1 repository の PR diff に安全に attach できないためです。位置を特定できる single-repo finding は、rule、severity、finding、AI agent への影響、利用可能な evidence、suggested fix を含む inline comment になります。最終 summary には health score/grade、severity distribution、inline と summary の件数、全 findings の index、各 finding の折りたたみ可能な完全な詳細、優先順位付き next steps が含まれ、安定した `<!-- ai-harness-doctor:pr-review -->` marker が付きます。再実行時、`--post` は complete comment を積み上げず、認証済み user または GitHub App が所有する最新 marker summary を更新します。foreign marker comment は編集しません。`--report PATH` を繰り返すと、1 回の dry-run または投稿で複数 report を組み合わせられます。inline finding は引き続き head-specific annotation です。GitHub が無効な inline 位置を HTTP 422 で拒否した場合、upsert 済みの完全な summary が fallback となり、再投稿はしません。permission、network、rate-limit、server error は引き続き明示されます。clean report の場合は、対象となった checks と action 不要であることを明示します。デフォルトは dry-run（JSON payload を出力し、ネットワークには一切触れません）で、`--post` 時のみ `GITHUB_TOKEN` を使って投稿します。2 つ目は **eval evidence + health gate** です。`evidence` を宣言した task から生成または再採点された result は、その fact source を deterministic な evidence metadata に自動で含めます。repeatable な `--evidence FILE` は `AGENTS.md` などの manual source を追加します。`eval --score ... --tasks ... --workdir ... --evidence AGENTS.md --require-current-evidence --fail-under <N>` は task-declared source を再導出し、missing/stale evidence を先に exit `7` で拒否してから既存 health threshold（exit `5`）を適用します。file evidence は SHA-256 を使い、directory evidence は child を recursive hash せず existence と type を bind します。fingerprint は byte identity または directory identity を証明するだけで、manual answer が real model 由来であることは証明しません。generic shipped guard では eval は optional のままですが、この repository の self-guard は committed result を必須にします。shipped guard command はすべて packaged CLI を通るため、`scripts/` tree をコピーしていない fresh consumer repository でも動作します。inline PR review feedback は GitHub 限定です。GitLab/Codebase テンプレートは scan と optional eval gate を共有しますが、provider-specific な inline comment は投稿しません。
...
2. **Every-PR gate** — `paths` / `paths-ignore` filter を使わず、hook bypass を防ぎます。security / semantic inputs には MCP/settings files、nested agent rules、ecosystem manifests、そして `AGENTS.md` が参照する任意の repository-relative path が含まれます。有限の allow-list では、この動的な D2/D7 surface を安全に網羅できません。
3. **Weekly checkup + lifecycle-managed issue** — pull request 間で進む遅い腐敗を防ぎます。environment change、dependency behavior の変化、repository edit なしで古くなる convention などです。failure が続く間は exact-title incident 1 件に evidence を追記し、最初の healthy run が recovery evidence をコメントして close します。無関係な issue には触れません。
...
すでにドリフトした repo に gate を導入したいですか。まず `drift --write-baseline FILE` で現在の findings を記録し、次に `drift --baseline FILE` で debt を new、known（`baselined`）、修復済み（`resolved_baseline`）に分類すれば、CI は新しい drift のみを gate します。baseline fingerprint は行番号に依存せず、known/修復済み debt は health に影響しないため、完全に baseline 化した repo でも grade A のままです。`--check-baseline` は修復済み entry の清掃が必要なら `9` で終了し、`--prune-baseline` は new debt を取り込まずその entry だけを atomic に削除します。
...
| MCP clients | `ai-harness-doctor mcp` は `harness_scan`/`drift`/`validate`/`plan`/`stubs`/`eval_generate`/`explain` を stdio 経由の MCP tools として公開します。 |
...
Adapters は `{{PLAYBOOK}}` を installed playbook path に置き換えます。copy payload は repository-owned baseline、rules、その他 state と分離された `.ai-harness-doctor/payload/` subtree に置かれます。version 付き `~/.ai-harness-doctor/manifest.json` は exact managed path と SHA-256 digest を記録します。既存 manifest が malformed、unsupported schema、または state path が symlink の場合、installer は fail closed して元の state を変更しません。valid state は atomic replacement で書き込みます。install/update/uninstall は owned process lock で直列化され、contained durable transaction journal を使います。catch された failure は filesystem changes と manifest を一緒に rollback し、process interruption の後は次の strict installer command が pre-manifest transaction を recovery するか、exact digest で既に commit 済みの manifest を認識します。malformed/ambiguous recovery や crash 後の external edit は fail closed し、evidence を上書きせず journal を保持します。これは logical journal recovery であり、cross-filesystem writes が 1 回の atomic rename を共有するという主張ではありません。install/update は unowned な同名 file や install 後に編集された managed file を上書きせず、`manual-merge` / `modified-preserved` として報告します。uninstall は byte 検証済み managed file だけを削除し、最後の参照 agent が削除されるまで shared payload を保持します。legacy manifest は additive に移行し、byte-identical な旧 payload file だけを退役させます。`--link` は payload files をコピーする代わりに global package を指します。CLI は安全でない `npx` cache linking をブロックし、先に global install するよう案内します。
...
|---|---|---|
| `github` | `.github/workflows/harness-drift.yml` every-PR gate + `.github/workflows/harness-checkup.yml` weekly scan/drift checkup with one lifecycle-managed issue。 | GitHub Actions で自動実行され、failure は exact incident を作成/更新し、recovery は close します。 |
...
**Self-bootstrap:** このリポジトリは自身の guard を実行します。`.github/workflows/harness-drift.yml` と `.github/workflows/harness-checkup.yml` は `assets/guard/` テンプレートから改変され、公開 package ではなくリポジトリの**ローカル** scan / drift 実装を実行するため、`scripts/` の変更は変更対象の code 自身に gate されます。review 済みの `.ai-harness-doctor/scan-baseline.json` は benchmark/test fixture に由来する既知 conflict を記録します。PR gate は新しい scan debt または drift で失敗します。generic shipped guard では eval は optional のままですが、このリポジトリは commit 済み self-eval に current task/`AGENTS.md` evidence と health を無条件で要求します。missing / limited token を許容するのは PR review 投稿 step だけです。
...
oversize instruction file では、`--max-bytes` は overlap、conflict、override、declaration analysis 用に保持する semantic text だけを制限します。inventory の SHA / line count と high-confidence secret / bypass checks は、file 全体を memory に保持せず、すべての byte を引き続き検査します。JSON は `analyzed_bytes`、`truncated`、`security_scanned_bytes`、top-level `analysis_limits` を公開し、Markdown は prefix-only の overlap evidence を明示します。bounded semantic analysis で finding が無いことを、未検査 tail に問題が無い証拠として扱いません。

**Structured rule applicability.** Scan は Claude Code `.claude/rules/**/*.md`、Cursor `.cursor/rules/**/*.mdc`、Copilot/VS Code `.github/instructions/**/*.instructions.md` を recursive に発見します。bounded な stdlib parser が Claude の `paths` block/inline string list（frontmatter なしは always-on）、Cursor の `alwaysApply`/`globs`/`description`、Copilot の `applyTo` をモデル化します。各 format に必要な documented bounded glob（brace alternatives と Claude character classes を含む）を扱いますが、general YAML parser ではありません。blocking conflict になるのは、異なる値の automatic domain が current contained repository path の少なくとも 1 つで重なる場合だけです。disjoint path rule は conflict せず、always-on/canonical rule と overlapping path rule は引き続き conflict します。conditional/semantic、manual、Cursor が無視する `.md`、malformed、truncated、no-current-match rule は `applicability` / `applicability_warnings` に表示されますが blocking conflict にはなりません。diagnostic は Markdown、SARIF、PR review に届きます。security、identity、overlap は applicability に関係なく original file を検査し、recursive discovery が Claude rule file の recursive deletion や rewrite を Treat に許可することはありません。Claude Code 自体が shared rule をサポートしていても、external rule symlink は repository audit boundary により除外されます。
...
**既存 scan debt の導入 baseline。** repository は現在の gap・semantic・conflict debt を一度記録し、透明な register として commit した後、新しい finding のみを gate できます。
...
version 付き baseline は deterministic で timestamp を含まず、finding family/rule、root または package scope、path/evidence、message を含む structured identity を使い、line number は identity ではなく evidence として扱います。現在の debt は new（active array）、known（`baselined`）、修復済み（`resolved_baseline`）に分類され、Markdown summary にも known/resolved 件数が表示されます。すべての baseline debt は fail-on decision と SARIF から除外されます。`--check-baseline` は削除可能な修復済み entry があれば `9` で終了し（active finding の 2/3/4/7 が優先）、`--prune-baseline` は修復済み entry だけを atomic に削除して new debt を取り込みません。HIGH security finding は baseline に入らず常に active で、手作業で追加した security 形式の entry も無視されます。通常 scan では missing / malformed baseline は何も抑制せず、明示的な check/prune は fail closed して書き込みません。Monorepo の root と package identity は区別されます。`--repos-file` は、無関係な各 repository が自身の debt register を所有すべきため全 baseline mode と意図的に非互換です。baseline の変更を code と同様に review してください。これは regex ignore list ではありません。
...
**ネストした指示スコープ。** conflict 診断は prose から推測せず、AGENTS.md の nearest-file rule に従います。各 `AGENTS.md` / `AGENT.md` の parent directory が lexical scope となり、各 config file は最も深い canonical ancestor に属します。同じ scope 内の異なる値だけが blocking `conflicts` になり、ancestor → descendant の差は non-blocking `scope_overrides`、sibling scopes は独立です。JSON は `instruction_scopes` と `scope_overrides` を追加し、Markdown と Treat plan も両方を表示するため、意図的な override は監査可能なまま root stub に潰されません。non-root の真の conflict は `scope` を baseline、SARIF、PR review まで保持します。file-type、glob/frontmatter、prose scope は推論しません。
...
**マルチリポジトリ・バッチモード。** `scan --repos-file PATH` は、単一の `repo_root` の代わりに `PATH` に列挙された各リポジトリ（1 行 1 パス、空行と `#` コメントは無視）をスキャンし、組織横断の health summary を出力します——これまで手動でリポジトリごとに実行する以外に手段のなかった "Mixed-tool team" / "OSS メンテナー" persona 向けです。各リポジトリはそれぞれ自身のルートで独立にスキャンされます（このモードではリポジトリ内の monorepo パッケージ展開は行いません）。ディレクトリとして解決できないパスは "Repos that could not be scanned" に列挙され、reachable repo の report は失われません。coverage は fail closed です。すべての reachable repo が完了した後、未スキャン entry が 1 件でもあれば final command は exit `8` になります。ただし finding gate の `2`（security）、`3`（gaps）、`4`（semantic）、`7`（conflicts）が優先されます。`--json` は `{ summary: { repo_count, error_count, aggregate }, repos: [{ path, resolved, name, has_agents_md, summary, report } | { path, resolved, error }] }` を返します。batch operational error は PR review では summary-only で、local `resolved` path を表示しません。`repo_root` positional argument および `--baseline` / `--write-baseline` とは排他的です。

**GitHub ネイティブな findings（SARIF）。** `scan` と `drift` はいずれも `--sarif` を受け付け、SARIF 2.1.0 ドキュメントを stdout に出力します。これにより findings が GitHub の Security タブや PR のインラインアノテーションに表示されます。`--sarif` は `--json`/markdown より優先され、`--no-*` output suppression に関係なく active report（root + すべての monorepo package）から生成されます。Scan SARIF には size/truncation warnings、security findings、gaps、semantic mismatches、conflicts、明示的に opt-in した custom-rule findings が含まれ、drift SARIF には built-in/custom drift findings が含まれます。file inventory、overlap candidates、nested-file inventory、project snapshot は code-scanning finding ではなく、JSON/Markdown の evidence のままです。正当に baseline 化された非 security debt は除外され、HIGH security finding は常に残ります。ソースレベルは SARIF レベルにマッピングされます（`HIGH`/`ERROR`→`error`、`MEDIUM`/`WARN`/`NOTICE`→`warning`、それ以外→`note`）。各 result は deterministic で line-insensitive な `partialFingerprints` エントリ（scan baseline が使うのと同じ canonical finding identity から導出）を持つため、無関係な編集や 1 行のずれがあっても同じ alert を維持し、古い alert を閉じて重複を新規作成することはありません。各コマンドの run はさらに自身の `automationDetails` category（`ai-harness-doctor/scan/` と `ai-harness-doctor/drift/`）を宣言するため、同じ commit に対して `scan` と `drift` の SARIF を両方アップロードしても、2 回目のアップロードが 1 回目のコマンドの alert を閉じることはなくなりました。deterministic な `properties.aiHarnessDoctor` property bag により、automation は command、error/warning/note/finding 件数、drift health（`ok`、score、grade）を直接読み取れます。
...
Action はデフォルトで選択した Action ref に同梱された実装を実行するため、`uses:` で指定したバージョンが実際に動くコードになります。別の npm バージョンまたはタグを意図的に使う場合に限り、任意の `version` input を設定してください。Action は Job Summary を書き、`status`（`ok`/`findings`/`maintenance`）、finding/error/warning/note 件数、`resolved-baseline-count`、drift の `health-score` / `health-grade` outputs を公開します。明示的に legacy npm version を選んだ場合も standard SARIF から件数を導出しますが、利用できない health outputs は空のままです。Doctor は一度だけ実行され、valid な non-zero finding または baseline-maintenance gate は SARIF、outputs、summary を先に公開してから元の CLI exit code を復元します。install/runtime/malformed-report failure が healthy result を捏造することはありません。この repository の required self-test は current bundled `scan` と `drift` を実行し、その後 public registry から解決した exact stable version で npm install override を検証します。published-package call は compatibility evidence であり、PR source の evidence は bundled call です。stable release は publish 前に両 bundled command を繰り返し、publish 後には floating Action で bundled scan と newly published exact npm drift override を実行します。各 success path は SARIF driver version を検証し、npm install を `RUNNER_TEMP` 内に限定します。
...
| `--fail-on-conflicts` | harness declaration の conflict があれば `7` で終了します。 |
| `--baseline FILE` | `FILE` に記録された gap/semantic/conflict debt のみを抑制します。security は抑制できません。 |
| `--write-baseline FILE` | 現在の非 security scan debt を deterministic baseline に書き込み、`0` で終了します。 |
| `--check-baseline` | `--baseline FILE` と併用し、削除可能な修復済み/stale entry があれば `9` で終了します（finding gate 優先）。 |
| `--prune-baseline` | `--baseline FILE` と併用し、修復済み entry だけを atomic に削除します。new finding は baseline 化しません。 |
...
| `--repos-file PATH` | `PATH` の各 repo を scan し、reachable reports を全て出した後、未スキャン entry があれば exit `8`（2/3/4/7 が優先）。`repo_root` と排他的です。 |
...
|---|---|
| Claude | `CLAUDE.md` / `.claude/CLAUDE.md` import `@AGENTS.md`。recursive に発見した `.claude/rules/**/*.md` は変更しません。 |
...
同じ考え方で、`adapters/` 配下のコマンド別 Codex/Cursor/Gemini アダプターは単一ソースから生成されます。`scripts/gen_adapters.py` が 1 つのコマンド表から全 18 ファイル（6 コマンド × 3 フレーバー）を描画し、`python3 scripts/gen_adapters.py --check`（`npm run lint:adapters` も同じ）は commit 済みのアダプターがそのソースからドリフトすると CI を失敗させ、`npm run gen:adapters` で再生成します。
...
Checks は extracted answer に対する `regex`、workdir で実行する `command`、または open-ended な LLM-as-judge grading のための `judge` にできます。task が pass するには runner が exit `0` で、選択した check も pass する必要があります。non-zero、timeout、spawn failure は bounded diagnostic failed record として残り、judge には送られません。明示的な external judge も exit `0` かつ valid passing verdict JSON が必要です。non-zero/malformed output は pass できず、別 judge へ silent fallback もしません。single、multi-round、matrix は同じ contract を使い、failure 後も later task/agent を続行します。overall CLI を fail させる場合は `--fail-under` または regression gate を使います。run、multi-round、matrix、regrade、strict score が他の処理を行う前に、task array 全体を検証します。各 task には unique かつ non-empty な `id`、non-empty な `prompt`、supported `check` object、さらに存在する場合は finite positive な `timeout_s` と有効な evidence/judge fields が必要です。invalid pack は簡潔な `task error` と exit `2` で停止し、runner、LLM/judge、evidence hash、result write、baseline update は一切発生しません。diagnostic に prompt や task content は表示されません。offline result consumer（`--score`、`--stats`、`--compare`、`--regrade`）は、読み取り後の副作用や書き込みの前に、保存済みの single-run、multi-round、matrix task records も個別に検証します。invalid input は簡潔な `result error` と exit `2` で停止します。Claude CLI JSON output の場合、grading は matching の前に `result` field を抽出します。Usage/cost fields は存在する場合に捕捉されます。`--compare before.json after.json` は Markdown comparison を書きます。`--regrade results.json --tasks tasks.json` は recorded outputs を offline で再採点します。`--workdir` を指定すると、run、matrix、regrade は task-level `evidence` に列挙された source を自動で stamp し、repeatable な `--evidence FILE` は manual/extra source を追加します。file は exact SHA-256 fingerprint を保ち、directory は recursive child bytes ではなく repository-relative path、existence、type のみを bind します。score 時の `--require-current-evidence` は同じ `--tasks` から task source を再導出し、repeat された explicit evidence と結合して、health check より先に stale result を exit `7` で拒否します。declared/explicit evidence のない hand-written task は legacy unstamped behavior を保ち、strict flag がなければ old unstamped result も引き続き score できます。runner binary がない場合、この command は実行したふりをせず、manual protocol fallback を表示します。
...
1 つの contained file、directory、future path に関連する instruction evidence を説明します。
...
schema-version-1 JSON は `target`、`effective_scope`、root→nearest の `canonical_chain`、`diagnostic_sources`、`source_applicability`、関連する `scope_overrides` / same-scope `conflicts`、明示的な `limitations` を含みます。既存 path と future path の両方を受け付け、contained absolute path は repository-relative path に正規化します。escape と external-symlink target は fail closed です。`.git`、`node_modules`、`dist`、`build`、`__pycache__` 配下の target は、その subtree 内の config が inventory 対象外なので `excluded_by_scan` と表示されます。

inheritance chain と呼ぶのは canonical file のみです。Claude `.claude/rules/**/*.md`、Cursor `.mdc`、Copilot `.instructions.md` では、`source_applicability` が上記 structured fields を使い、具体的な existing/future target を automatic/non-matching/conditional/manual/ignored/invalid と決定的に分類します。description-based semantic selection、malformed metadata、other tools、prose scope は引き続き**診断上の関連付け**のみで、effective とは主張しません。この command は instruction text の merge、plugin 実行、file 変更を行いません。
...
- `tools/list` → `harness_scan`、`harness_drift`、`harness_validate`、`harness_plan`、`harness_stubs`、`harness_eval_generate`、`harness_explain` を closed input schema 付きで広告します。
...
Python は優先順位順に検出されます: `AI_HARNESS_DOCTOR_PYTHON`、次に `PYTHON`、次に `python3`、次に `python`。Python **3** インタプリタのみが受け入れられます。存在しない場合、すべての Python 子コマンド（`scan`、`explain`、`plan`、`validate`、`stubs`、`drift`、`review`、`eval`）は、raw stack trace ではなく同じ明確で実行可能なメッセージ（Python 3 をインストールするか `AI_HARNESS_DOCTOR_PYTHON` を設定する）で失敗します。
