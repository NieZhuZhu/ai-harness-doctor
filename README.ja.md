...
version 付き baseline は deterministic で timestamp を含まず、finding family/rule、root または package scope、path/evidence、message を含む structured identity を使い、line number は identity ではなく evidence として扱います。抑制された debt は top-level `baselined` array と Markdown baseline summary に残りますが、fail-on decision と SARIF からは除外されます。HIGH security finding は baseline に入らず常に active で、手作業で追加した security 形式の entry も無視されます。baseline が missing / malformed の場合は何も抑制しません。Monorepo の root と package identity は区別されます。`--repos-file` は、無関係な各 repository が自身の debt register を所有すべきため scan baseline と意図的に非互換です。baseline の変更を code と同様に review し、debt の修正に合わせて縮小してください。これは regex ignore list ではありません。
