...
|---|---:|---:|---|
| `scan` | ✅ | ❌ | デフォルトで 0 で終了します。inventory、evidence、security checkup、不足しているインフラの gap analysis、および技術スタックの project snapshot（オプションの `--agent-gaps` agent 推論フック付き）を行います。`--fail-on-security` は HIGH の findings があれば 2 で終了し、`--fail-on-gaps` は ERROR の gap があれば 3 で終了します。 |
...
`AI_HARNESS_DOCTOR_SKIP=1` は local hook の明示的で監査可能な escape hatch です。`guard --remove --apply` は managed snippets を削除し、**すべての provider** の CI files をクリーンアップし（provider を切り替えても何も残りません）、可能な場合は byte-exact な既存 hook content を復元します。
...
さらに **gap analysis** を実行し、リポジトリを harness 完全性チェックリストと diff して、*不足している*必須インフラ（既存のものだけでなく）を報告します。これらの静的チェックは、スタックに関係なくどんな健全な harness にも必要な部分のみを対象とします。canonical なルート `AGENTS.md`（`G1`）、必須の `AGENTS.md` セクション（`assets/AGENTS.template.md` と同期、`G2`）、`AGENTS.md` を指す最小 pointer であるべき tool stub（`G3`）、drift-guard / 週次 checkup の CI workflow（`G4`）です。各 gap は `level`（`ERROR`/`WARN`/`NOTICE`）、`item`、`message`、実行可能な `suggestion` を持ちます。`--fail-on-gaps` を付けると、ERROR レベルの gap（例: ルート `AGENTS.md` の欠落）が 1 つでもあれば `3` で終了します。
...
かつて静的な `G5`–`G8` gap だったスタック依存の判断（pre-commit guard、maintenance contract、MCP 設定、permission 設定）は、この snapshot の事実となり、agent 推論に委ねられます。

**Agent 推論（`--agent-gaps CMD`）** は project snapshot を外部の agent/LLM コマンドにパイプし、スタック固有の gap（「CI のない Go module」「lint 設定のない Node リポジトリ」など）を推論させます。snapshot と静的 gap は JSON（`{"project_snapshot": {...}, "gaps": [...]}`）としてコマンドの **stdin** に渡されます。コマンドは gap オブジェクトの JSON 配列（または `agent_gaps` リストを持つオブジェクト）を出力する必要があります。結果は `agent_gaps` key に追加されます。失敗は `{"agent_gaps": {"error": "…"}}` として捕捉され、scan がクラッシュすることはありません。
...
| `--agent-gaps CMD` | snapshot を stdin JSON として `CMD` に渡し、推論された gap を `agent_gaps` に追加します。 |
...
`security` の findings は `level`（`HIGH`/`MEDIUM`）、`category`（`secret`/`mcp`/`permission`/`hook`/`instruction`）、`path`、および人間が読める `message` を持ちます。`--no-security` では `security` key は省略されます。`gaps` の entries は `check`（`G1`–`G4`）、`level`（`ERROR`/`WARN`/`NOTICE`）、`item`、`message`、`suggestion` を持ちます。`--no-gaps` では `gaps` key は省略されます。`--no-snapshot` では `project_snapshot` が省略され、`agent_gaps` は `--agent-gaps` を指定した場合のみ出力されます。
