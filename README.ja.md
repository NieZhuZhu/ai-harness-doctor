...
**Self-bootstrap:** このリポジトリは自身の guard を実行します。`.github/workflows/harness-drift.yml` と `.github/workflows/harness-checkup.yml` は `assets/guard/` テンプレートから改変され、公開 package ではなくリポジトリの**ローカル** scan / drift 実装を実行するため、`scripts/` の変更は変更対象の code 自身に gate されます。review 済みの `.ai-harness-doctor/scan-baseline.json` は benchmark/test fixture に由来する 5 件の既知 conflict を記録します。PR gate は新しい scan debt または drift で失敗します。eval gate は soft のままで、missing / limited token を許容するのは PR review 投稿 step だけです。
