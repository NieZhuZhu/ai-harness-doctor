...
schema-version-1 JSON は `target`、`effective_scope`、root→nearest の `canonical_chain`、`diagnostic_sources`、関連する `scope_overrides` / same-scope `conflicts`、明示的な `limitations` を含みます。既存 path と future path の両方を受け付け、contained absolute path は repository-relative path に正規化します。escape と external-symlink target は fail closed です。`.git`、`node_modules`、`dist`、`build`、`__pycache__` 配下の target は、その subtree 内の config が inventory 対象外なので `excluded_by_scan` と表示されます。

effective inheritance chain と呼ぶのは canonical file のみです。Cursor、Copilot、Claude などの認識済み config は**診断上の関連付け**に限定され、effective とは主張しません。この command は tool 固有の glob/frontmatter/prose applicability を推論せず、instruction text の merge、plugin 実行、file 変更も行いません。
