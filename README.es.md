...
- **Arreglar el harness mejora al agente de forma medible.** En el benchmark reproducible antes/después, consolidar una configuración contradictoria en un único `AGENTS.md` canónico elevó las respuestas objetivas correctas de **6/28 a 28/28**, eliminó las dos respuestas inestables y redujo la latencia media un 27% y el coste registrado un 17% en tareas idénticas — [metodología y datos brutos](benchmark/README.md).
- **Se sostiene en un repositorio real de producción.** En trycompai/comp (un monorepo bun de ~1.7k estrellas en desarrollo activo cuyo `CLAUDE.md` se había bifurcado de `AGENTS.md` y aún enseñaba comandos `npx` obsoletos), una sola ejecución del tratamiento llevó al agente de **18/24 a 24/24 correctas** — antes repetía el documento obsoleto textualmente, de forma determinista, en ambas ejecuciones — recortando además la latencia un 37% y los turnos del agente un 45%; el límite se documenta con honestidad mediante un **resultado nulo** publicado sobre openai/codex, cuyos documentos sanos no dejaban nada que mejorar — [caso positivo](benchmark/corpus/evals/comp/README.md) · [caso nulo](benchmark/corpus/evals/codex/README.md).
- **La enfermedad es real en la élite del open source.** Un corpus de 14 repositorios conocidos (react, vscode, n8n, ollama, transformers, dify, supabase, gemini-cli, codex, home-assistant, zed, elasticsearch, cline, ghostty — 58k–247k estrellas, fijados como submodules) muestra que 10/14 ya incluyen un `AGENTS.md` raíz y, aun así, un único escaneo determinista por lotes revela **96 carencias, 44 archivos de instrucciones solapados, 15 discrepancias declaración-código y 3 conflictos de mismo ámbito** en 150 archivos de configuración de agentes — [corpus y resultados por repo](benchmark/corpus/README.md).
- **Hallazgos en los que puedes confiar.** 38 rondas de validación registradas contra repositorios reales han producido quince correcciones de falsos positivos (openai/codex, microsoft/vscode, cline, gemini-cli, assistant-ui, …), cada una publicada con tests de regresión; las clases aplazadas se registran abiertamente en lugar de ocultarse — [registro de validación externa](EXTERNAL_VALIDATION.md).
...
Comprueba qué versión resolvió npx:
...
Apply/remove del guard es transaccional para el hook de Git, los archivos del provider y `AGENTS.md`.

Los fallos capturados revierten, las mutaciones interrumpidas se recuperan antes del siguiente `--apply` y un estado de recuperación inseguro o modificado externamente falla de forma cerrada.
...
- Los artefactos e informes eval redactan credenciales de alta confianza en diagnósticos runner/judge, metadata usage anidada y templates matrix; conservan usage numérico y la evaluación usa en memoria la salida acotada original.
