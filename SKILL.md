...
The same template can run an **eval evidence + health gate**. Add repeatable `--evidence FILE` during run/matrix/regrade to stamp deterministic task/evidence SHA-256 metadata. At score time, pass the same `--tasks`, `--workdir`, evidence list, and `--require-current-evidence`: stale/missing evidence exits 7 before `--fail-under <N>` applies the existing health gate (exit 5). Fingerprints prove byte identity, not that manual answers came from a real model. Without strict freshness, legacy unstamped results remain scoreable; generic guard templates therefore keep eval optional, while this repository's self-guard requires its committed result.
...
**Evidence freshness.** A high stored score is meaningful only for the inputs that produced it. With `--workdir`, repeat `--evidence FILE` to stamp the exact tasks file and repository-relative evidence files into run, matrix, or regraded JSON (`schemaVersion: 1`, logical paths, SHA-256 digests; no absolute paths or file contents). `--score RESULTS --tasks TASKS --workdir REPO --evidence AGENTS.md --require-current-evidence` recomputes that manifest and exits 7 on a changed task/evidence byte, missing metadata, or a different evidence set before printing/gating health. Evidence files must resolve inside workdir; external symlinks and escapes are rejected. The flag is explicit so old result files stay usable for historical reporting.
