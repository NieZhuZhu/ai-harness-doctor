...
- `--json` machine output with `files` (complete identity plus semantic/security coverage), `warnings`, `applicability`, `applicability_warnings`, `analysis_limits`, `overlaps`, `conflicts`, `nested`, `surface` (MCP/subagents/commands/hooks/permissions), `security` (severity-ranked findings), and `custom` (findings from user rule plugins). With `--baseline` it additively includes `baseline` and attributed `baselined` debt; in monorepo mode it also includes `packages` and `monorepo`.
