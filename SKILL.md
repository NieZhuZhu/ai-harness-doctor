...
- The repository contains multiple rule files, such as `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.cursor/rules/*.mdc`, `.windsurfrules`, `.github/copilot-instructions.md`, `GEMINI.md`, `.clinerules`, `.amazonq/rules/*.md`, `.junie/guidelines.md`, `WARP.md`, `.idx/airules.md`, `.goosehints`, `.kiro/steering/*.md`, `.augment-guidelines`, or `.kilocode/rules/*.md`.
...
python3 scripts/scan.py /path/to/repo --min-maturity guarded  # exit 5 below maturity level 3 (opt-in gate)
python3 scripts/scan.py /path/to/repo --no-maturity        # hide the maturity ladder section
...
The repository-root composite `action.yml` wraps the same SARIF path for GitHub Actions. Its default `version: bundled` executes the implementation shipped in the selected Action ref, so `uses: owner/repo@tag` pins the code that actually runs; an explicit npm version/tag remains available as an override. It emits a Job Summary plus status/severity-count/resolved-baseline/drift-health outputs from deterministic SARIF producer metadata. Prefer `args-json` (a JSON string array) for exact/repeated extra argv or values containing spaces; legacy `args` remains first-line spaces/tabs splitting, the two inputs are mutually exclusive, and neither is shell-evaluated. A Node 16 stdlib helper validates bounded argv and executes both bundled/npm CLIs with `shell:false`; `action-report.js` remains the sole SARIF/output parser. The doctor runs once: a valid non-zero findings or baseline-maintenance gate publishes the report before restoring the exact CLI exit, while invalid commands, installation/runtime errors, and malformed reports never fabricate a healthy result. A legacy npm override derives counts from standard SARIF and leaves unavailable health outputs empty. Required self-test covers current bundled `scan`/`drift` plus an exact already-published npm override; the first two prove PR source, while the latter proves install compatibility. Release automation derives a floating tag from the package major (`1.x` → `v1`), runs both bundled commands before npm publish, then checks out the remote major tag and verifies its bundled scan plus the exact newly published npm drift override after publish. Every success path checks the SARIF driver version and keeps npm installation under `RUNNER_TEMP`; older major tags are preserved.

The scan checks the configuration-file inventory, size warnings, overlap candidates, conflict candidates, and nested `AGENTS.md` files. Signal-conflict detection additionally suppresses slash-joined supported-tool enumerations (`npm/yarn/pnpm`, `jest/vitest/mocha`, `prettier/biome/dprint`) — a line that lists supported managers, test frameworks, or formatters never manufactures a conflict against the real declaration elsewhere in the same file. It also inventories the **extended harness surface** — MCP servers, subagents, slash commands, hooks, and permission rules — and runs a **security checkup** that flags plaintext secrets, overly broad permission rules (e.g. `Bash(*)`, `bypassPermissions`), insecure MCP transports, and risky hook bodies (`curl … | bash`, `rm -rf`, `--dangerously-skip-permissions`). It additionally runs a **semantic consistency** check (via `scripts/semantic.py`) that cross-checks the concrete claims in `AGENTS.md` — build/test commands, repo-relative paths, the package manager, and language/runtime versions (Node.js, Python, Go, Rust, Java, Ruby, .NET) — against ground truth in `package.json`, `Makefile`, the filesystem, lockfiles, `.nvmrc` / `engines.node`, `go.mod`, `Cargo.toml`, `pom.xml` / `build.gradle`, `Gemfile`, `.csproj` / `global.json`, and their equivalents, surfacing declaration-vs-code mismatches at checkup time.

**Harness maturity ladder.** The report also assembles existing signals into a `maturity` section answering "what is still missing before this repository is genuinely harnessed, and what do I run next": Level 0 *Ungoverned* → 1 *Inventoried* (any recognized instruction file) → 2 *Canonicalized* (root `AGENTS.md` with required sections, no draft markers, tool files as minimal pointer stubs) → 3 *Guarded* (a drift gate wired into CI plus the maintenance contract) → 4 *Evidenced* (an eval health gate in CI), mirroring the four phases. Levels are cumulative and definitional — which artifacts exist, detected deterministically (file presence, the guard marker, and line-level command-form CI invocations only; prose mentions never count) — not a judgement: the section carries no severity, and every stack-dependent signal (MCP config, permissions, local pre-commit hooks, debt baselines, the weekly checkup workflow) sits in a clearly labeled advisory list that never affects the level. Each missing required item names the packaged command that installs it. The section never changes the default exit code; the opt-in `--min-maturity LEVEL` gate (1–4 or a level name) exits `5` when the root repository's level is lower, with precedence security > gaps > maturity > semantic > conflicts. Maturity is a status report, not findings: it is absent from SARIF, baselines, and PR review, and monorepo *package* sub-reports deliberately carry no ladder (guard/eval CI is a repository-level concept), while `--repos-file` batch tables add a per-repo Maturity column.
...
The scanner's filesystem boundary is the audited repository root: repository-derived configs, manifests, workspace metadata, semantic facts, and default rule plugins are used only when their resolved targets remain inside that root. Embedded fixture/demo repositories under `tests/fixtures/*`, `test/fixtures/*`, `benchmark/repo-before/*`, `benchmark/repo-after/*`, and `benchmark/corpus/repos/*` are treated as test data when auditing the parent repository and are skipped unless scanned directly as the target root. In-repo file symlinks stay supported and retain their lexical repo-relative report path. Explicit `--rules DIR` locations remain an intentional opt-in and may live outside the repository.

Semantic/D2 path checks treat an absent path as deliberate when repository-owned `.gitignore` files positively ignore its exact repository-relative name. Matching uses one bounded Git query with synthetic metadata, so nested rules and negations work without consulting the target `.git/config`, `.git/info/exclude`, user/global config, or an enclosing worktree. Git absence, warnings, malformed output, timeouts, unsafe symlinks, and temporary-metadata failures suppress nothing: the existing missing-path finding remains. This is deterministic checkout-absence evidence, not mutation authority or a general ignore language. The same path checks also skip a two-segment `org/name` backtick token when adjacent same-line words explicitly label it as a Docker/OCI image (`image`, `docker image`) or an RPC/API method (`rpc method`, `endpoint`, `operation`): those are runtime identifiers, not files. They also skip bounded, fail-closed non-path identifiers when prose explicitly labels them as linter rules (for example, oxlint/ESLint `react/exhaustive-deps`) or as branch refs (for example, `current branch is gitbutler/workspace`). They also skip GitHub Actions `uses` references that share the `owner/repo` shape of a path: the reserved first-party `actions/` org (`actions/checkout`, `actions/setup-node`, `actions/upload-artifact`) suppresses on its own, while any other `owner/repo` needs an explicit same-line `uses:` cue, and pinned forms (`actions/checkout@v4`) are already handled by the `@` guard. On multi-language monorepos the same checks also classify positively-evidenced non-path tokens: short-form Go import paths matching a fact-chain `go.mod` require/module suffix (`Shopify/sarama`, `uber/fx`, `sourcegraph/conc/pool`), Go stdlib imports (`net/http`) and dotless exported-symbol shapes (`remote/RemoteBus`) in go.mod scopes, npm dependency subpath imports (`next/link`), dotted code symbols (`core/agent.Agent`), slash-separated value enums (`low/medium/high`, `linux/amd64`, `.ts/.tsx`, `GetTools/Compose/Run`), CJK-cued branch examples (`示例：feat/agent_memory`), and repository-name-prefixed paths whose remainder exists. The exclusions are bounded and fail-closed — an explicit filesystem cue (`file`, `directory`, `edit`, ...) always wins; absent manifest evidence, an unlabeled `org/name` and any extension-bearing or three-plus-segment token (e.g. `docker/compose.yml`) stays a checked path; and every manifest-evidenced import classification is additionally suppressed only when the token has no local anchor (its first segment exists as a directory in no fact-chain scope).
...
**Nested instruction scopes.** Conflict detection implements the AGENTS.md nearest-file hierarchy mechanically: canonical file parent directories are lexical scopes, configs belong to their deepest canonical ancestor, same-scope disagreements are blocking `conflicts`, and ancestor → descendant differences are non-blocking `scope_overrides` (siblings are independent). JSON adds `instruction_scopes` and `scope_overrides`; Markdown and Treat plans preserve override evidence, while only true scoped conflicts enter baselines, SARIF, PR review, and exit `7`. Root conflict shape stays compatible; non-root conflicts add `scope`. Natural-language file-type, external-repository, and prose scope is never inferred.

**Structured Claude/Cursor/Copilot applicability.** Scan recursively discovers Claude Code `.claude/rules/**/*.md`, Cursor `.cursor/rules/**/*.mdc`, and Copilot/VS Code `.github/instructions/**/*.instructions.md`. A bounded standard-library parser supports Claude `paths` block/inline string lists (no frontmatter means always-on), Cursor `alwaysApply` / `globs` / `description`, and Copilot `applyTo`, with format-bounded glob syntax including brace alternatives and Claude character classes. It is not a general YAML parser. Different values become blocking conflicts only when their automatic domains overlap on a current contained repository path; disjoint path rules are independent. Conditional/manual, ignored Cursor `.md`, malformed/truncated, and no-current-match rules are non-blocking `applicability_warnings` delivered through Markdown/SARIF/PR review. Security, complete identity, and overlap still inspect original bytes. Recursive read discovery never expands Treat's mutation authority: Claude rule files remain untouched, and external rule symlinks remain outside the repository audit boundary even though Claude Code itself supports shared rules.

**Adopting scan gates with existing debt.** `--write-baseline FILE` records the current gap, semantic, and conflict findings as a deterministic, timestamp-free debt register; `--baseline FILE` classifies debt as new (active arrays), known (`baselined`), or repaired (`resolved_baseline`) while excluding all baseline debt from fail-on gates and SARIF. Stable identities include the family/rule, root-or-package scope, path/evidence, and structured values; line numbers are evidence, never identity. `--check-baseline` exits `9` when repaired entries need removal (active gates retain precedence); `--prune-baseline` atomically removes only repaired entries and never absorbs new debt. HIGH security findings are structurally excluded. A missing/malformed baseline suppresses nothing in ordinary mode, while explicit check/prune fails closed without writing. Commit and review the register—never use it as an opaque ignore list. Monorepos preserve root/package attribution; `--repos-file` rejects baseline composition because each unrelated repository must own its own register.
...
finding exits retain precedence (`2` security, `3` gaps, `5` maturity gate,
`4` semantic, `7` conflicts). Batch operational errors are summary-only in PR review and use a
safe repository label rather than the local resolved path. Batch mode rejects
`--sarif` (exit 1, actionable stderr, no output) before scanning: per-repository
SARIF is not emitted, so use `--json` for the aggregate machine-readable report.

**SARIF finding completeness.** `scan --sarif` maps every active finding family—size/truncation warnings, applicability diagnostics, security, gaps, semantic mismatches, conflicts, and explicitly enabled custom rules—at root and monorepo-package scope. `drift --sarif` maps built-in and custom drift findings. Baselined debt is excluded; inventory, overlap candidates, nested-file inventory, and project snapshots remain JSON/Markdown evidence rather than findings. Every result carries a deterministic, line-insensitive `partialFingerprints` entry (from the same canonical identity scan baselines use) so alerts survive unrelated edits, and each command declares its own `automationDetails` category (`ai-harness-doctor/scan/` vs `ai-harness-doctor/drift/`) so uploading both SARIF files for one commit does not let the second close the first's alerts.
...
- `--json` machine output with `files` (complete identity plus semantic/security coverage), `warnings`, `applicability`, `applicability_warnings`, `analysis_limits`, `overlaps`, `conflicts`, `nested`, `surface` (MCP/subagents/commands/hooks/permissions), `security` (severity-ranked findings), `maturity` (the harness maturity ladder), and `custom` (findings from user rule plugins). With `--baseline` it additively includes `baseline` and attributed `baselined` debt; in monorepo mode it also includes `packages` and `monorepo`.
...
**Nested drift scopes.** D5 itself remains non-blocking inventory, but every contained nested `AGENTS.md` it discovers is also guarded by D1/D2/D6/D7. D1/D2/D6 reuse the same contained nearest-first lexical ancestor chain as targeted eval: package-local scripts, paths, Node pins, and lockfiles are considered before repository-root fallbacks, while sibling packages never supply scripts/paths/runtime facts. Explicit root scripts remain valid; a nested canonical scope may describe suffix paths in its own descendants; the existing parent-scope dependency-binary fallback is retained. One narrow path exception: a missing token whose trailing suffix has exactly ONE source in the whole repository and no local anchor (its first segment exists under no fact-chain directory) is treated as an unambiguous cross-subtree reference (`cmd/enterprise` named from `deploy/`), not drift; ambiguous or locally anchored names stay findings. D7 is deliberately different because Markdown targets stay relative to the canonical file. Findings carry the nested canonical-file path into strict health, baselines, `--fix` manual attention, SARIF, and PR review. D3/D4/D8 and custom plugins still run once at repository scope. Historical root findings keep their implicit `AGENTS.md` path, so scope-less v1 baseline entries remain compatible.
...
A repo that already carries drift cannot flip on `--strict` without a wall of red. `--write-baseline FILE` records current findings once, then `--baseline FILE` classifies debt as new, known (`baselined`), or repaired (`resolved_baseline`) so CI gates only new drift. Fingerprints are `(check, message, path)` and ignore line numbers. Known/repaired debt never affects score or normal exit; a fully baselined repo remains grade A. `--check-baseline` exits `9` when repaired entries should be removed (active drift/min-score exits keep precedence), while `--prune-baseline` atomically removes only repaired entries and never records new findings. Missing/malformed ordinary baselines suppress nothing; explicit check/prune fails closed without writing. Commit and review the deterministic register.
...
python3 scripts/check_drift.py /path/to/repo --baseline .ai-harness-doctor/drift-baseline.json --check-baseline
python3 scripts/check_drift.py /path/to/repo --baseline .ai-harness-doctor/drift-baseline.json --prune-baseline
```
...
Runner stdout/answer/stderr, external-judge raw/reason/stderr, nested usage
metadata, persisted matrix runner templates, and comparison reports redact the
same high-confidence credential shapes as scan reports before any result
JSON/Markdown is written. Numeric usage/cost and container shape remain
available. Detection, usage parsing, answer extraction, and grading still use
the original bounded output in memory, so redaction does not change pass/fail.
Offline regrade sanitizes the records it rewrites and deterministically grades
the stored (possibly already redacted) stdout; this is artifact minimization,
not encryption or general PII classification.
...
Persist each run's health as an append-only **baseline history** (a JSON list of snapshots) so you can track efficacy over time and gate on regressions. `--save-baseline` appends the current run's health to `--baseline FILE` (recording timestamp, label, score/grade, pass counts, and the target repo's git commit/branch when available). `--check-regression` compares the current score to the most recent prior snapshot and **exits 6** when the score drops by at least `--regression-threshold` points (default `5`). `--trend FILE` renders the recorded history as a markdown table with per-snapshot deltas and regression flags. These flags compose with any run mode (`--tasks`, `--rounds`, `--matrix`) and with `--score` on an existing results file. The history store is validated before any trend/regression/append read, exactly like the other offline result consumers: a structurally malformed history (not a JSON array of snapshots, or a snapshot whose `score` is a non-number) exits `2` with a concise `result error` and no traceback, and never appends onto a corrupt file. A missing/`null` score remains a valid, non-comparable snapshot; regression and trend math run only over validated numeric snapshots.
...
Schema version 1 reports the normalized `target`, `effective_scope`, root→nearest `canonical_chain`, `diagnostic_sources`, `source_applicability`, relevant `scope_overrides` / same-scope `conflicts`, and `limitations`. Existing and future contained paths are accepted; escapes and external symlinks are rejected. Targets under scanner-excluded subtrees are explicitly marked. Canonical files are standards-backed inheritance. Claude `.claude/rules/**/*.md`, Cursor `.mdc`, and Copilot `.instructions.md` are classified for the concrete target from the deterministic structured controls above; description-selected/manual/malformed/other-tool/prose applicability stays diagnostically associated only. It never merges text, executes plugins, or writes files.
