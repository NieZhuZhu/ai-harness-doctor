...
**SARIF finding completeness.** `scan --sarif` maps every active finding family—size/truncation warnings, applicability diagnostics, security, gaps, semantic mismatches, conflicts, and explicitly enabled custom rules—at root and monorepo-package scope. `drift --sarif` maps built-in and custom drift findings. Baselined debt is excluded; inventory, overlap candidates, nested-file inventory, and project snapshots remain JSON/Markdown evidence rather than findings.
