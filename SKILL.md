...
**Structured Cursor/Copilot applicability.** Scan recursively discovers Cursor `.cursor/rules/**/*.mdc` plus Copilot/VS Code `.github/instructions/**/*.instructions.md`. A bounded standard-library parser supports the documented scalar controls: Cursor `alwaysApply` / `globs` / `description` and Copilot `applyTo`, including quoted/unquoted strings, booleans, comma-separated patterns, and bounded brace alternatives. It is not a general YAML parser. Different values become blocking conflicts only when their automatic domains overlap on a current contained repository path; disjoint path rules are independent. Conditional/manual, ignored Cursor `.md`, malformed/truncated, and no-current-match rules are non-blocking `applicability_warnings` delivered through Markdown/SARIF/PR review. Security, complete identity, and overlap still inspect original bytes. Recursive read discovery never expands Treat's top-level-only Cursor deletion authority.
...
Schema version 1 reports the normalized `target`, `effective_scope`, root→nearest `canonical_chain`, `diagnostic_sources`, `source_applicability`, relevant `scope_overrides` / same-scope `conflicts`, and `limitations`. Existing and future contained paths are accepted; escapes and external symlinks are rejected. Targets under scanner-excluded subtrees are explicitly marked. Canonical files are standards-backed inheritance. Cursor `.mdc` and Copilot `.instructions.md` are classified for the concrete target from the deterministic structured controls above; description-selected/manual/malformed/other-tool/prose applicability stays diagnostically associated only. It never merges text, executes plugins, or writes files.
