...
| 39 | [wshobson/agents](https://github.com/wshobson/agents) + [wasp-lang/open-saas](https://github.com/wasp-lang/open-saas) + [vstorm-co/pydantic-deepagents](https://github.com/vstorm-co/pydantic-deepagents) | 2026-07-20 | Three active AI/adopter repositories selected for a read-only full four-stage validation round, prioritizing real AI harness files and excluding previously verified browser-use/browser-use | 1 false positive found and fixed: Cursor `.mdc` frontmatter with documented inline YAML sequences such as `globs: ["**/*.py", "**/pyproject.toml", "**/uv.lock"]` was rejected as unsupported YAML, making path-scoped Cursor rules invalid and hiding their signal text. After the fix, both affected wshobson/agents rules classify as path-scoped with match counts. open-saas and pydantic-deepagents produced expected canonical-root gaps only; no engine bug. | This PR |
| 40 | [agent-sh/agnix](https://github.com/agent-sh/agnix) + [johnpapa/ai-ready](https://github.com/johnpapa/ai-ready) + [PanisHandsome/ai-rules-sync](https://github.com/PanisHandsome/ai-rules-sync) | 2026-07-21 | Three active AI-harness repositories with AGENTS/Copilot-style config, selected for a read-only full four-stage validation round while avoiding the previously verified browser-use/browser-use and other logged repositories | 1 Phase-3 reporting bug found and fixed: unsupported/empty generated eval packs were scored as `0/100 F, 0/0 tasks passed`, which looked like a failed agent run instead of insufficient efficacy evidence. `eval` health now marks empty task sets as unscored (`score: null`, `grade: N/A`, `insufficient_evidence: true`) and `--fail-under` still fails closed. agnix additionally surfaced genuine fixture drift under its own test corpus; ai-ready and ai-rules-sync had expected canonical-section gaps only. | This PR |
| 41 | [openai/openai-agents-js](https://github.com/openai/openai-agents-js) + [carlrannaberg/claudekit](https://github.com/carlrannaberg/claudekit) + [yzhao062/anywhere-agents](https://github.com/yzhao062/anywhere-agents) | 2026-07-22 | Three active AI/AI-harness repositories with AGENTS.md or equivalent rich agent config, selected while avoiding every previously verified repository listed above | 2 false positives found and fixed from openai-agents-js: (a) `pnpm changeset` was reported as an unknown script even though `@changesets/cli` supplies the `changeset` binary; (b) `local-npm:publish` and "local npm registry" were misread as declarations that the repo uses npm, creating a bogus pnpm↔npm package-manager conflict. After fixes, openai-agents-js scan is clean for security/semantic/conflicts and drift is 95/A with only a D4 size notice. claudekit and anywhere-agents produced useful real/deferred drift evidence but no additional tool bug was fixed. | This PR |
| 42 | [danny-avila/LibreChat](https://github.com/danny-avila/LibreChat) + [mendableai/firecrawl](https://github.com/mendableai/firecrawl) + [lobehub/lobe-chat](https://github.com/lobehub/lobe-chat) | 2026-07-23 | Three active AI-application repositories with real root `AGENTS.md` + `CLAUDE.md` (lobe-chat also ships `GEMINI.md`, nested `*/CLAUDE.md`, and a 95-package pnpm workspace), selected for a read-only full four-stage validation round while avoiding every previously verified repository listed above | 1 false positive found and fixed from lobe-chat: the ecosystem nouns "npm packages" (registry packages) and "npm scripts" (package.json `scripts` entries) in "`bunx` for executable npm packages" and "`bun` to run npm scripts" were misread as declarations that the repo installs deps with npm, manufacturing a bogus npm-vs-pnpm/bun package-manager conflict (the repo's `packageManager` is pnpm, with bun as the script runner). After the fix, lobe-chat's only remaining package-manager signal is the genuine, human-actionable pnpm(deps)↔bun(scripts) split. LibreChat's D8 competing-lockfile ERROR (both `bun.lock` and `package-lock.json` committed at root) and firecrawl's canonical-section gaps are correct, expected findings, not tool bugs. | This PR |

**Loop status:** rounds 1-42 complete. Round 42 ran the full four-stage chain across danny-avila/LibreChat, mendableai/firecrawl, and lobehub/lobe-chat, fixing one lobe-chat package-manager false-positive class: the ecosystem nouns "npm packages" (registry packages) and "npm scripts" (package.json `scripts` entries) are no longer read as declarations that the repo installs deps with npm, so a pnpm+bun monorepo no longer manufactures a bogus npm-vs-pnpm/bun conflict. LibreChat's committed dual lockfiles (`bun.lock` + `package-lock.json`) and firecrawl's canonical-section gaps were confirmed as correct, expected findings. Round 41 ran the full four-stage chain across openai/openai-agents-js, carlrannaberg/claudekit, and yzhao062/anywhere-agents, fixing two OpenAI Agents JS false-positive classes: dependency-binary aliases such as `@changesets/cli` → `changeset` are accepted for pnpm/yarn/bun passthrough checks, and package-manager conflict extraction no longer treats `npm` inside script names or npm-registry prose as an npm usage declaration. Round 40 ran the full four-stage chain across agent-sh/agnix, johnpapa/ai-ready, and PanisHandsome/ai-rules-sync and fixed a Phase-3 reporting bug: empty generated eval task packs are now unscored (`N/A`) instead of reported as `0/100 F`, while `--fail-under` still fails closed for insufficient evidence. Round 39 fixed Cursor `.mdc` inline YAML sequence parsing from wshobson/agents. Round 38 ran the full four-stage chain across assistant-ui/assistant-ui, elie222/inbox-zero, and google/adk-python and fixed two Phase-0/Phase-2 path false-positive classes surfaced by assistant-ui — prose-labelled linter rule ids (`react/exhaustive-deps`, `react/rules-of-hooks`) and a non-prefixed branch ref named by a strong equative cue (`gitbutler/workspace` in "the current branch is …") — plus a pre-existing CI-red README regression (all seven README pre-commit `rev:` pins were left at `v1.13.1` while `package.json` shipped `1.13.2`); released as v1.13.3. inbox-zero surfaced a deferred "example path" class (`utils/example.test.ts` behind an "e.g.") and adk-python came back clean (drift 100/A). Round 37 completes the efficacy bracket opened by round 36: disease-targeted sampling found trycompai/comp, whose stale `CLAUDE.md` (forked from `AGENTS.md`, still teaching `npx` on a bun monorepo) produced deterministic wrong answers that one Phase-1 treatment run eliminated (18/24 → 24/24, −37% latency), and logged five deferred false-positive candidates from the 26-repo sweep. Round 36 publishes the first real-repo Phase-3 before/after eval (openai/codex at the corpus pin) as an honest null result — task-level metrics saturate when the only doc defects are stale path references in otherwise-healthy docs — and logs that the round-32 runtime-identifier closure does not cover the real codex sentence (RPC cue 54 chars from the token, outside the 40-char window; widening deferred). Rounds 33-35 (langchain-ai/langgraph, RooCodeInc/Roo-Code, mem0ai/mem0) ran the full four-stage chain and fixed two false-positive classes — an example git branch name (`feature/my-new-feature`) misread as a missing path by both the Phase-0 semantic scan and the Phase-2 D2 `--strict` gate, and a self-referential import-stub suggestion that told users to reduce the canonical `AGENTS.md` to a stub pointing at itself; released as v1.12.2. Round 32 (bugfix) also closes the previously-logged `org/name` runtime-identifier class — Letta's `letta/letta` Docker image (logged in round 28) and OpenAI Codex's `thread/read` / `app/list` RPC-method examples (retained from round 14/31) — via one bounded same-line context classifier shared by Phase-0 and Phase-2 D2. Round 11 (v0.15.2) added a dotenv false-positive fix found by running the full four-stage chain against OpenHands; rounds 12-13 (litellm, openai-agents-python) came back clean; rounds 14-15 (v0.15.3) fixed one shared subdirectory-scoped-path false-positive class found across openai/codex and sst/opencode. Round 16 (v1.1.2) fixed two conflict-detection false-positive classes — the complementary ESLint+Prettier formatter pair (google-gemini/gemini-cli, block/goose) and existence-negated tool names (cline/cline). Rounds 17–19 validate nested scan scope, nested drift, and target-path explain semantics against current Mastra; round 20 validates complete identity/security coverage plus honest bounded-semantic evidence on a real oversize AGENTS.md; round 21 validates bounded explicit-scope efficacy generation on current Mastra; round 22 proves generated fact sources are automatically freshness-bound; round 23 proves the complete task pack fails before any paid runner side effect; round 24 proves batch coverage stays complete but cannot return a false green when one listed repo is absent; round 25 validates deterministic path/conditional applicability and target explain behavior on current first-party GitHub and VS Code rule sets; rounds 26–27 validate Claude Code block-list and always-on project rules on current Bitwarden and Algolia checkouts. Round 28 (v1.9.1) fixed a `test_command` conflict false positive — a package-manager test command (`npm`/`pnpm test`) plus the framework it invokes (`vitest`) — reproduced by running the full four-stage chain across QwenLM/qwen-code and langgenius/dify. Round 29 closes the deferred Qwen gitignored-runtime-path class while preserving the independent `scripts/copy-assets.ts` drift; round 30 closes Dify's lexical package-ancestor class without changing unrelated sparse findings.

## Round 41 detail — openai/openai-agents-js, claudekit, anywhere-agents

Ran the dev checkout's four-stage chain (`scan --json`, `canonicalize.py --plan`, `check_drift.py --json`, and `eval_run.py --generate`) against three fresh public repositories. openai-agents-js started with 0 security findings, 1 semantic mismatch, 1 package-manager conflict, and drift 80/B; after the fixes it has 0 semantic mismatches, 0 conflicts, and drift 95/A with only the non-failing D4 context-bloat notice. claudekit generated 10 eval tasks and produced real/deferred drift evidence, including missing docs links and symlinked tool stubs; no engine change was made for those because the findings describe target-repo state or a larger symlink-policy question. anywhere-agents generated an empty eval pack because no supported Node/Go/Python project facts exist in that source repo, and its scan/drift findings are mostly source-vs-consumer bootstrap path references such as `.agent-config/` and `.claude/skills/`; no quick false-positive fix was shipped for that broader documentation-mode distinction.

**FP 1 — dependency package name was treated as the only possible binary name.** openai-agents-js documents `pnpm changeset`, and its root `package.json` declares `@changesets/cli`. Because the checkout has no `node_modules/.bin`, the fallback passthrough check only compared the command token with dependency package names, so `changeset` was incorrectly reported as an unknown script by both Phase 0 semantic scan and Phase 2 D1 drift. The fix adds a bounded package-to-binary alias map shared by `semantic.py` and `check_drift.py`; `@changesets/cli` now contributes the `changeset` binary while unknown commands are still flagged.

**FP 2 — package-manager extraction matched npm inside script names and registry prose.** The same OpenAI Agents JS file mentions `pnpm local-npm:publish` and a "local npm registry". Both are npm-registry/script-name context, not a declaration to use npm as the repo package manager, but the conflict detector matched the standalone `npm` substring and manufactured a pnpm↔npm conflict. The fix tightens the npm package-manager signal so it remains a real token but ignores word characters, underscores, and hyphens before `npm`, plus explicit `npm registry` prose. Regression tests keep real `npm test` vs `pnpm install` conflicts intact.
