...
- Baselines are visible debt registers: HIGH security is ineligible; fingerprints are deterministic/root-package-aware and ignore evidence line numbers. Shrink repaired debt. Every report family needs PR-review/SARIF traversal; preserve package paths, keep batch findings summary-only, and never post baselined debt as active.
- MCP tools stay read-only; sync closed schemas, exit policies, stdio tests, README/SKILL entries. Findings are not operational failures.
