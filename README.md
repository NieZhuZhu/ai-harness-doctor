...
By default, the Action runs the implementation bundled with the selected Action ref, so the `uses:` version is the code that actually executes. Set the optional `version` input only when you intentionally want to run a different npm version or tag. The Action writes a Job Summary and exposes `status`, finding/error/warning/note counts, plus drift `health-score` / `health-grade` outputs; a selected legacy npm override still derives counts from standard SARIF but leaves unavailable health outputs empty. It executes the doctor once: a valid non-zero findings gate publishes SARIF, outputs, and summary before restoring the exact CLI exit code, while installation/runtime/malformed-report failures never fabricate a healthy result. This repository's required self-test runs current bundled `scan` and `drift`, then checks the npm-install override against an exact stable version resolved from the public registry; that published-package call is compatibility evidence, while the bundled calls are the evidence for PR source. Stable releases repeat both bundled commands before publish and, afterward, run the floating Action with its bundled scan plus an exact newly published npm drift override. Every success path validates SARIF driver version and confines npm installation to `RUNNER_TEMP`.
