...
**Self-bootstrap:** this repository runs its own guard. `.github/workflows/harness-drift.yml` and `.github/workflows/harness-checkup.yml` are adapted from the `assets/guard/` templates to run the repo's **local** scan and drift implementations instead of the published package, so changes to `scripts/` are gated by the code being changed. The reviewed `.ai-harness-doctor/scan-baseline.json` records five known conflicts contributed by benchmark/test fixtures; the PR gate fails on any new scan debt or drift. The eval gate remains soft, and only the PR-review posting step tolerates a missing/limited token.
