...
- **Findings you can trust.** 38 logged validation rounds against real repositories have produced fifteen false-positive fixes (openai/codex, microsoft/vscode, cline, gemini-cli, assistant-ui, …), each released with regression tests; deferred classes are logged openly instead of hidden — [external validation log](EXTERNAL_VALIDATION.md).
...
| Security | Plaintext secrets (AWS, GitHub, OpenAI, Anthropic, Google, OpenRouter, Groq, xAI, Perplexity, HuggingFace, GitLab, npm, PyPI, Slack, Stripe, Tavily, DigitalOcean, Doppler, SendGrid credentials and JWTs), broad permissions, insecure MCP transports, dangerous hook bodies, and bypass guidance. |
...
- Eval task concurrency is explicit and bounded with `--parallel N` (`1`–`32`, default `1`); result order remains deterministic.
