...
- **Findings you can trust.** 37 logged validation rounds against real repositories have produced thirteen false-positive fixes (openai/codex, microsoft/vscode, cline, gemini-cli, …), each released with regression tests; deferred classes are logged openly instead of hidden — [external validation log](EXTERNAL_VALIDATION.md).
